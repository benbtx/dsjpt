...
<script>
  ...
  export default {
    ...
    created () {
        ...
        if (this.currentMenus.length === 0) {
          //重新获取 currentMenus
          // this.currentMenus=this.lsMenus;
          // this.$store.dispatch('changeCurrentMenu',{ path: location.pathname,fullPath:location.pathname});
          //方法2  把vuex存到 localstorage 中 ，然后长度为0 时 重新赋值 
          if(localStorage.currentMenus!=undefined){
              var lsmenus=JSON.parse(localStorage.currentMenus );
              this.$store.commit('LOAD_CURRENT_MENU', lsmenus);
          }
