        ...
        <el-row style="width: 100%;">
          ...
          <el-col :span="6">
              <div class="el-input" style=" float: left; ">
                通话次数：
                 〉
                <el-input style='width:80px' v-model="thcs" placeholder="10000"></el-input>
          ...
          <el-col :span="8">
              <!-- <el-button type="primary" icon="plus" @click="btnGetGXTPJS()">最短路径</el-button> -->
              <el-button type="primary" icon="plus" @click="btnGetGXTPJS">关系图谱计算</el-button>
  ...
  export default {
    ...
    methods: {
                ...
                window.onresize = function () {
                  ...
                  resizeTimer = setTimeout(function () {
                    // that.drawchart('chart');
                      // that.chart.resize({width:,height:});
                       that.chart.resize({width:that.chart.getDom().clientWidth,height:that.chart.getDom().clientHeight});
      ...
      drawchart(id) {
        ...
        // this.chart.resize({width:this.chart.getDom().clientWidth,height:this.chart.getDom().clientHeight});
        // this.chart.resize({width:200,height:200});
        this.chart.resize();
      ...
      btnGetGXTPJS(){
        ...
        var _this=this;
          ...
          // if(this.graphVo==null||this.twoWayMappingId1==0||this.twoWayMappingId2==0){ this.$message('请先选中两个节点'); return;}
           ...
           if(this.thcs==''){ this.$message('请输入通话次数'); return;}
         
        //每次计算先清空 通过先输50 查询 再输 60 查询 ，第一次结果未清除
        for (const item of option.series[0].data) {
            //选中节点不变色 
              if(item.data.twoWayMappingId==_this.twoWayMappingId[0]){
                continue;
              }
            item.itemStyle.color='#409EFF'
        ...
        for (const item of option.series[0].links) {
          ...
          .getGXTPJS({twoWayMappingId:this.twoWayMappingId[0],param:this.thcs,archiveNum:this.archiveNum})
